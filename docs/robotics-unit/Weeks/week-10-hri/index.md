...
At the **cognitive level**, humans are goal-directed agents. They form *beliefs* about the world, have *desires* or objectives, and take *actions* to achieve them. This **BDI (Beliefs, Desires, Intentions)** framework gives us a mental model of the human as a rational agent — much like our robot. The key insight is that if we can infer what a person *intends*, we can predict their future behaviour and respond cooperatively.
...
For richer intent modelling, we can treat human motion as the result of a **policy** — just like our robot. Given a goal and a belief about the world, what trajectory would a rational agent choose? This leads to **inverse reinforcement learning** approaches that infer the reward function driving human behaviour from observed demonstrations. Once we have that reward function, we can use it to predict future motion.
